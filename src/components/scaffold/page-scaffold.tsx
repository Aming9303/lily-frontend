...
export function PageScaffold({ route, dynamicLabel }: PageScaffoldProps) {
  return (
    <main className="surface rounded-[var(--radius-lg)] p-8 sm:p-10">
      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="rounded-[var(--radius-sm)] border border-[var(--color-line)] bg-[var(--color-panel-muted)] px-4 py-3 font-mono text-sm text-[var(--color-muted)]">
          {dynamicLabel ?? route.path}
        </div>
      </div>

      <section className="mt-[var(--spacing-section)] grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <article className="rounded-[var(--radius-md)] border border-[var(--color-line)] bg-[var(--color-panel-muted)] p-6">
        ...
        <article className="rounded-[var(--radius-md)] border border-[var(--color-line)] bg-[var(--color-panel-muted)] p-6">
          ...
          <ul className="mt-4 grid gap-3">
            {route.implementationAreas.map((area) => (
              <li
                key={area}
                className="rounded-[var(--radius-sm)] border border-[var(--color-line)] bg-white px-4 py-3 text-sm text-[var(--color-muted)]"
              >
                {area}
              </li>
            ))}
